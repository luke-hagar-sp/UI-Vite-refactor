...
import { isRFC5646Step, serializeRFC5646 } from './models/rfc-5646';
...
const deserializers: Record<string, Deserializer> = {
  ...
  nameNormalizer: deserializeNameNormalizer,
...
export function deserializeToStep(data: any): Step {
  ...
  const deserializer = deserializers[stepType];
...
export class TransformBuilderComponent implements OnInit, OnDestroy {
  ...
  private editorDialog = inject(MatDialog);
  ...
  isNewTransform = signal<boolean>(false);
  showDesigner = signal<boolean>(true);
  ...
  defaultStepEditorProvider?: StepEditorProvider;
  ...
  constructor() {
    ...
    effect(() => {
      ...
      // Update designer theme if it exists
      if (this.designer) {
        const themedDesigner = this.designer as ThemedDesigner;
        themedDesigner.setTheme?.(isDark ? 'dark' : 'light');
      }
    });
  }
  ...
  public readonly stepsConfiguration: StepsConfiguration = {
    iconUrlProvider: (componentType, type) => {
      const iconMap: Record<string, () => string> = {
        ...
        randomNumeric: getRandomNumericIcon,
  ...
  public ngOnDestroy(): void {
    // No need to unsubscribe from themeSub as it's not a signal
  }
  ...
  public onDefinitionChanged(definition: Definition) {
    ...
    // No need to call autoSaveSubject.next(definition) as it's not a signal
  }
  ...
  // Add method to force designer recreation when theme changes
  private forceDesignerRecreation(): void {
    if (this.designer) {
      // Temporarily hide the designer
      this.showDesigner.set(false);

      // Use setTimeout to ensure the DOM updates before showing again
      setTimeout(() => {
        this.showDesigner.set(true);

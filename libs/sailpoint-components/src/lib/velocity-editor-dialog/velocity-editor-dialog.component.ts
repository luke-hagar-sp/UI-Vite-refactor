...
import { AfterViewInit, ChangeDetectorRef, Component, ElementRef, Inject, OnDestroy, OnInit, ViewChild, inject, signal, effect } from '@angular/core';
...
export class VelocityEditorDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Services
  private dialogRef = inject(MatDialogRef<VelocityEditorDialogComponent>);
  ...
  editorStats = signal<any>('{"lines":0,"length":0,"selection":0,"cursor":"1:1"}');
  currentTheme = signal<'light' | 'dark'>('light');
  wordWrapEnabled = signal(true);
  identities: any;

  constructor(@Inject(MAT_DIALOG_DATA) public data: VelocityEditorData) {
    this.originalCode = data.code || '';
    this.currentTheme.set(data.theme || 'light');
    ...
    this.editorView = new EditorView({
      ...
      dispatch: (tr) => {
        ...
        if (tr.docChanged) {
          ...
          this.hasChanges.set(currentValue !== this.originalCode);

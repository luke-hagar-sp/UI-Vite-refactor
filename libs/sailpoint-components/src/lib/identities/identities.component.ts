...
export class IdentitiesComponent implements OnInit {
  ...
  identities = signal<IdentityV2025[] & Record<string, unknown>[]>([]);
  filteredIdentities = signal<IdentityV2025[] & Record<string, unknown>[]>([]);
  ...
  async loadIdentities() {
    ...
    this.cdr.detectChanges(); // Force change detection for Angular 20
    ...
    try {
      ...
      this.identities.set((response.data ?? []) as IdentityV2025[] & Record<string, unknown>[]);
  ...
  async onRemoteSearch(query: string): Promise<void> {
    ...
    try {
      ...
      this.filteredIdentities.set((identities ?? []).map(
      ...
      this.totalCount.set(this.filteredIdentities().length);
  ...
  toggleSort(displayColumn: string): void {
    ...
    if (existingIndex > -1) {
      ...
      this.sorters.set([...this.sorters(), isAsc ? `-${apiField}` : '']);
      if (!this.sorters()[existingIndex]) this.sorters.set(this.sorters().filter((_, i) => i !== existingIndex));
    } else {
      this.sorters.set([...this.sorters(), apiField]);
    }
  ...
  private initializeColumns(): void {
    if (this.identities().length > 0) {
      this.allColumns.set(Object.keys(this.identities()[0]));
      this.columnOrder.set([...this.allColumns()]);

      if (!this.columnOrder().includes('viewAction')) {
        this.columnOrder.set([...this.columnOrder(), 'viewAction']);
      }
      ...
      if (!this.displayedColumns().includes('viewAction')) {
        this.displayedColumns.set([...this.displayedColumns(), 'viewAction']);
      }
    }
  }
}
